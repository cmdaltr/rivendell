...
# NOTE: Individual win_elastic test doesn't exist, using win_splunk_elastic_nav (all SIEM exports)
set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEST_DIR="$(dirname "$(dirname "$SCRIPT_DIR")")"
cd "$TEST_DIR"
BATCH_NAME="4b-elastic-export"
...
echo "BATCH 4b COMPLETE - Passed: $passed, Failed: $failed"
