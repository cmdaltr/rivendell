...
"""Volatility 3 macOS plugin output parser for Rivendell."""
...
def macos_vol3(
    ...
    """Parse Volatility 3 macOS plugin JSON output.

    Args:
        volver: Volatility version (should be "3")
        profile: macOS profile/symbol table
        symbolorprofile: Label for profile field
        plugin: Plugin name (e.g., "mac.pslist.Pslist")
    ...
    try:
        ...
        print(f"    [ERROR] Failed to parse Volatility 3 macOS output for {plugin}: {e}")
        import traceback
        traceback.print_exc()
