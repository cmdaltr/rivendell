...
def select_files(
    output_directory, verbosity, d, mnt, img, vssimage, collectfiles, recover=False
):
    if collectfiles:
        stage = "collecting"
    print(
    ...
    elif isinstance(collectfiles, str) and collectfiles not in ("True", "False", "include:", "exclude:") and not collectfiles.startswith("include:") and not collectfiles.startswith("exclude:"):
        # Selection string passed (e.g., "R" for archives, "RD" for archives+documents)
        file_selection = collectfiles
    else:
        file_selection = "A"
    if "N" not in file_selection:
        ...
        if (
            ...
            # BUGFIX: Disabled automatic archive extraction during collection phase
            # Archives should only be collected, NOT automatically extracted
            # Extraction should only occur during processing phase if explicitly requested
            # if os.path.exists(
