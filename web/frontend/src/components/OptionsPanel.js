  ...
  const steps = [
    {
      ...
      type: 'single',
      options: [
        { key: 'super_quick', label: 'Super Quick', description: 'Minimal processing for the fastest results', time: -40 },
        { key: 'quick', label: 'Quick', description: 'Faster processing by skipping some hash calculations', time: -20 },
  ...
  const renderConfirmStep = () => {
    ...
    steps.forEach(step => {
      if (step.options) {
        step.options.forEach(opt => {
          if (options[opt.key]) {
            selectedOptions.push({ step: step.title, label: opt.label });
          }
        });
      }
    });
        ...
        <div className="confirm-summary">
          {selectedOptions.map((item, index) => (
            <div key={index} className="confirm-item">
              <span className="confirm-step-name">{item.step}:</span>
              <span className="confirm-option-name">{item.label}</span>
            </div>
          ))}
        </div>
      ...
      <div className="wizard-content">
        ...
        <div className="wizard-navigation">
          ...
          <div className="wizard-actions-right">
            {currentStepData.type === 'confirm' && onSubmit && (
              <button
                type="button"
                onClick={onSubmit}
