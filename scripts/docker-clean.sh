...
show_disk_usage() {
    ...
    docker system df 2>/dev/null || echo -e "${YELLOW}(Unable to get disk usage - corrupted snapshots may exist)${NC}"
    echo ""
}
...
stop_containers() {
    ...
    cd "$PROJECT_ROOT"
    docker-compose -f build/docker/docker-compose.yml down 2>/dev/null || true
    docker-compose -f src/analysis/docker/docker-compose.yml down 2>/dev/null || true
...
# Function to verify Docker is running and check for issues
check_docker() {
    echo -e "${BLUE}Checking Docker health...${NC}"
    ...
    if ! docker system df >/dev/null 2>&1; then
        echo -e "${YELLOW}⚠ Warning: Corrupted Docker snapshots detected${NC}"
        echo -e "${YELLOW}  This will be fixed during cleanup...${NC}"
    else
        echo -e "${GREEN}✓ Docker storage is healthy${NC}"
    fi

    # Check for dangling build cache
    CACHE_SIZE=$(docker builder prune -a -f --dry-run 2>/dev/null | grep "Total:" | awk '{print $2}' || echo "0B")
    if [ "$CACHE_SIZE" != "0B" ]; then
        echo -e "${YELLOW}⚠ Build cache found: ${CACHE_SIZE}${NC}"
    else
        echo -e "${GREEN}✓ Build cache is clean${NC}"
    fi

    # Check for stopped containers
    STOPPED_CONTAINERS=$(docker ps -a -q -f status=exited 2>/dev/null | wc -l | tr -d ' ')
    if [ "$STOPPED_CONTAINERS" -gt 0 ]; then
        echo -e "${YELLOW}⚠ ${STOPPED_CONTAINERS} stopped container(s) found${NC}"
    else
    ...
    if [ "$DANGLING_IMAGES" -gt 0 ]; then
        echo -e "${YELLOW}⚠ ${DANGLING_IMAGES} dangling image(s) found${NC}"
    else
...
show_menu() {
    ...
    echo "  6) Health Check (Check Docker system health)"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  -c, --health      Check Docker system health and exit"
            echo "  -h, --help        Show this help message"
            echo ""
            echo "Examples:"
            echo "  $0 --quick                # Quick clean"
            echo "  $0 --deep --rebuild       # Deep clean and rebuild"
            echo "  $0 --health               # Check Docker health only"
...
if [ -z "$LEVEL" ]; then
    while true; do
        ...
        case $choice in
            ...
            6) check_docker; continue;;
            7) show_disk_usage; continue;;
...
echo -e "${BLUE}║  Starting: $(printf '%-44s' "${LEVEL_UPPER} CLEAN")  ║${NC}"
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo ""
...
if [[ "$rebuild" == "yes" ]]; then
    ...
    if docker-compose -f build/docker/docker-compose.yml build --no-cache; then
        echo ""
        echo -e "${GREEN}✓ Build completed successfully!${NC}"
        echo ""
        echo -e "${YELLOW}Start services with:${NC}"
        echo -e "${BLUE}  docker-compose -f build/docker/docker-compose.yml up -d${NC}"
    else
        echo ""
    ...
    echo -e "${BLUE}  docker-compose -f build/docker/docker-compose.yml build --no-cache${NC}"
fi
